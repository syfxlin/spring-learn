...
public abstract class Helper {
  protected static ConversionService convert = ApplicationProvider
    .get()
      ...
      if (keys[i].equals("*")) {
        ...
        if (target.isObject()) {
          ...
          for (final Iterator<String> it = object.fieldNames(); it.hasNext();) {
            ...
            final JsonNode element = dataGet(
              object.get(itemKey),
              subKeys,
              null,
              JsonNode.class
  ...
  public static <T> T dataGet(
    ...
    if (keys == null) {
      return convert.convert(target, returnType);
    }
    for (int i = 0; i < keys.length; i++) {
      if (keys[i].equals("*")) {
        ...
        return convert.convert(array, returnType);
      } else if (target instanceof List) {
  ...
  public static String strRandom(final int length) {
